...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: User.userDidLogoutNotification), object: nil, queue: OperationQueue.main) { (Notification) -> Void in
            // whenever anyone posts user did logout as a notification, then this code is run
            // grab story board and send it back to that login view
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
    ...
    // hands you back the URL, that has my app ouath
